...
from ..models import AcademicSession, ActiveStudent, CAScore, ExamResult, Subject, Term
...
class CAScoreViewSet(viewsets.ModelViewSet):
    ...
    def bulk_upload(self, request):
        ...
        with transaction.atomic():
            ...
            for idx, row in enumerate(rows, start=2):
                try:
                    ...
                    if ca1_score > 15 or ca2_score > 15:
                        errors.append(
                            {"row": idx, "error": "CA score exceeds max (15)"}
                        )
                        continue
                    ...
                    subject, subj_created = _get_or_create_subject(
                        subject_name, subjects_map
                    )
                    if subj_created:
                        subjects_created += 1
            ...
            if to_update:
                CAScore.objects.bulk_update(
                    to_update, ["ca1_score", "ca2_score", "uploaded_by"]
                )

        invalidate_score_cache(session.id, term.id)
        logger.info(
            f"CA scores uploaded: {created} created, {updated} updated by {request.user.username}"
        )
...
class ExamResultViewSet(viewsets.ModelViewSet):
    ...
    def _enrich_with_bf_data(self, serialized_data, result_objects, session_id):
        ...
        for i, item in enumerate(serialized_data):
            ...
            item["cumulative_score"] = (
                round(sum(term_totals) / len(term_totals), 2) if term_totals else None
            )
        ...
        with transaction.atomic():
            ...
            for idx, row in enumerate(rows, start=2):
                try:
                    ...
                    total_questions = int(
                        row.get("total_questions", "30").strip() or "30"
                    )
                    ...
                    if obj_score > 30:
                        errors.append(
                            {"row": idx, "error": "OBJ score exceeds max (30)"}
                        )
                        continue
                    ...
                    if existing:
                        ...
                        new_obj = ExamResult(
                            ...
                            ca2_score=ca2_score,
                            obj_score=obj_score,
                            total_obj_questions=total_questions,
                            uploaded_by=request.user,
                        )
                        to_create.append(new_obj)
        ...
        return Response(
            {
                ...
                "updated": updated,
                "subjects_created": subjects_created,
                "missing_ca_scores": missing_ca[:10],
                "failed": len(errors),
                "errors": errors[:10],
            }
        )
        ...
        with transaction.atomic():
            ...
            for idx, row in enumerate(rows, start=2):
                try:
                    ...
                    if theory_score > 40:
                        errors.append(
                            {"row": idx, "error": "Theory score exceeds max (40)"}
                        )
                        continue
    ...
    def recalculate_positions(self, request):
        ...
        return Response(
            {
                "success": True,
                "message": "Totals and positions recalculated",
    ...
    def export_template_obj(self, request):
        ...
        response = HttpResponse(content_type="text/csv")
        response["Content-Disposition"] = (
            'attachment; filename="obj_scores_template.csv"'
        )
    ...
    def export_template_theory(self, request):
        ...
        response = HttpResponse(content_type="text/csv")
        response["Content-Disposition"] = (
            'attachment; filename="theory_scores_template.csv"'
        )
...
def bulk_upload_ca_scores(request):
    ...
    invalidate_score_cache(session.id, term.id)
    logger.info(
        f"CA scores uploaded: {created_count} created, {updated_count} updated by {request.user.username}"
    )
            ...
            try:
                ...
                if ca_obj:
                    ...
                    missing_ca_scores.append(
                        {"admission_number": admission_number, "subject": subject_name}
                    )
...
def _recalculate_totals(session, term):
    ...
    in a session/term. Needed because bulk_create/bulk_update bypass model save().
    """
...
def _calculate_class_positions(session, term, class_level_id=None):
    ...
    for key, group_results in class_subject_groups.items():
        sorted_results = sorted(
            group_results, key=lambda x: float(x.total_score or 0), reverse=True
        )
